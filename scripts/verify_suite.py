...
# Skill content lives under skills/j-space in this extension package.
ROOT = os.path.join(os.path.dirname(HERE), "skills", "j-space")
...
def main():
    ...
    if fronted != ["SKILL.md"]:
        fail(
            "tree",
            "frontmatter must appear only in SKILL.md, found: %s"
            % ", ".join(sorted(fronted)),
        )
    ...
    if entry_text.startswith("---\n"):
        ...
        if unsupported:
            fail(
                "SKILL.md", "unsupported frontmatter keys: %s" % ", ".join(unsupported)
            )
    ...
    for name in REFERENCES:
        ...
        if not os.path.exists(os.path.join(ROOT, rel)):
            ...
            rel.replace(os.sep, "/") not in entry_text
            and (name + ".md") not in entry_text
        ):
    ...
    if findings:
        ...
        return 1
    print(
        "verify_suite: clean — one entry, one premise, nine modules, no version talk."
    )
    return 0
