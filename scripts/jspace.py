    ...
    for line in lines:
        ...
        if current and head:
            if current in ("Goal", "Next"):
                ...
                book[current].append(
                    head[2:].rstrip() if head.startswith("- ") else head.rstrip()
                )
    return book
...
def read_history():
    ...
    try:
        ...
        print(
            "WARNING: history was unreadable and has been restarted (%s)." % exc,
            file=sys.stderr,
        )
        return []
    ...
    if not valid:
        print(
            "WARNING: history had an invalid shape and has been restarted.",
            file=sys.stderr,
        )
        return []
    return hist
...
def append_history(book):
    ...
    if problem:
        print(
            "WARNING: recent seam history was not saved — " + problem, file=sys.stderr
        )
    return hist
...
def observations(hist):
    ...
    if len({h["next"] for h in run}) == 1 and run[0]["next"]:
        found.append("Your next action has been the same for %d seams." % STALL_RUN)
    if run[0]["verified"] == run[-1]["verified"]:
        found.append("Nothing new has been verified across those %d seams." % STALL_RUN)
...
def print_ledger(book):
    ...
    if len(core) > 2:
        print(
            "          (+%d more in the ledger — two live at a time)" % (len(core) - 2)
        )
...
def mode_seam(book):
    ...
    if found:
        ...
        print(
            "If that is depth, carry on. If it is a stall, the moves open to you are:"
        )
        print("  " + SHIFTS + ".")
    if not one(book, "Next"):
        print()
        print(
            "There is no next action recorded. The ledger stops being state at that point."
        )
    return 0
...
def mode_note(book, args):
    ...
    if args.core:
        if "—" not in args.core and " - " not in args.core:
            ...
            if idx > len(live):
                ...
                refused.append(
                    (
                        "that core entry is already live.",
                        "choose the slot that should actually change",
                    )
                )
            elif idx == len(live):
        ...
        refused.append(
            (
                "--core-slot requires --core.",
                '--core "name — defining fact" --core-slot 1',
            )
        )
    ...
    if args.check:
        ...
        if args.by and "check" not in invalid:
            refused.append(
                (
                    "--by requires --check.",
                    '--check "what now holds" --by "verifier and coverage"',
                )
            )
    ...
    if args.open:
        ...
        refused.append(
            ("--settled-by requires --open.", '--open "question" --settled-by "test"')
        )
    ...
    if args.close is not None:
        ...
        idx = next(
            (i for i, row in enumerate(rows) if row.startswith(target + " ")), None
        )
        if idx is None:
            refused.append(
                (
                    "no open question numbered %d." % args.close,
                    "run `seam` to see the list",
                )
            )
            ...
            refused.append(
                (
                    ...
                    '--close %d --check "what now holds" --by "verifier and coverage"'
                    % args.close,
                )
            )
        else:
    ...
    if changed:
        ...
        if problem:
            ...
            print(
                "  free the path, or work without the file: keep the five lines in the"
            )
