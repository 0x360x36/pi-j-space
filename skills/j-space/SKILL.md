...
`<skill-root>` resolves to this skill's directory; the **jspace** tool runs the controller
and prints the premise, the full ledger, the invariants, and the prompt for step 4 (action
`resume`). `seam` prints the same full anchor when it detects a long gap. Without the
...
The `jspace` tool knows one thing you cannot know accurately: what state you were in a few
seams ago. It keeps the record and hands it back. It decides nothing, and it blocks nothing.

It runs in the task workspace: the ledger is written to `.jspace/` in the current directory, so
it stays with the task rather than with the skill.

```text
jspace action seam                      # the ledger, plus what has and has not moved since
jspace action note goal "..." next "..."   # open the ledger
jspace action note ...                  # record a checkpoint, question, swap, or next action
jspace action ship file PATH            # register check on anything about to leave
jspace action resume                    # premise, invariants and full ledger, after a gap
```

(The `ship` action also takes `text` instead of a file path — the equivalent of piping
stdin through the original CLI's `ship -`.)

The actions are named for moments, not for passes, so this is the mapping — a lookup, not a
...
| **loop** | `note` with `goal` and `next` to open the ledger, `seam` at every seam, `note` at each checkpoint, `ship` before delivery, `resume` after any long gap. |

It refuses only what it could not do — a checkpoint with no record does not get written,
because a ledger you cannot trust is worse than no ledger. It never stops you from working.
